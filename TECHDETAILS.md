...
- Whenever CS2 Love has a valid Intiface URL but no live WebSocket - whether because the initial connect failed (Intiface not yet running) or because a previously-good connection dropped - the session controller schedules a fresh `connect()` attempt 2 seconds later. The retry runs on every GUI tick (~10 Hz) and continues indefinitely until either a connect succeeds or the user edits the URL. Committing a new URL cancels any pending retry.
